...
PYTHONPATH=backend/src uv run celery -A stock_platform.workers.celery_app:celery_app worker --pool=solo --concurrency=1 --queues=control,ingestion-low,research-ingestion,stream-events,agent-research,agent-portfolio,agent-alert,agent-review,celery --loglevel=INFO
