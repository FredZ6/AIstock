...
def test_incremental_fills_advance_persisted_order_status(engine: Engine) -> None:
    ...
    first_batch = simulator.execute(order, (bar(1, "30"),))
    second_batch = simulator.execute(order, (bar(2, "100"),), prior_fills=first_batch)
    ...
    with engine.connect() as connection:
        ...
        store.persist(order, first_batch, entries)
        assert (
            connection.execute(
        ...
        store.persist(order, second_batch, entries)
        assert (
            connection.execute(
                select(paper_order.c.status).where(paper_order.c.id == order.id)
            ).scalar_one()
        ...
        connection.execute(
            text(
                ...
                INSERT INTO order_intent (
                    id, portfolio_id, symbol, side, quantity, decision_time,
                    execution_policy_version_id, risk_approved
                ) VALUES (:id, :portfolio_id, 'NVDA', 'BUY', 10, :decision_time, :policy_id, true)
                """
            ),
            {
                "id": order_id,
