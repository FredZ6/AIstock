"""Preserve idempotent fill redelivery before cumulative validation."""

from collections.abc import Sequence

from alembic import op

revision: str = "0012_idempotent_fill_guard"
down_revision: str | None = "0011_unique_paper_fill_reversal"
...
def upgrade() -> None:
    op.execute(
        ...
        DECLARE
            ...
            existing_fill paper_fill%ROWTYPE;
        ...
        BEGIN
            ...
            SELECT * INTO existing_fill
            FROM paper_fill
            WHERE idempotency_key = NEW.idempotency_key;
            IF FOUND THEN
                IF NEW.id IS DISTINCT FROM existing_fill.id
                   OR NEW.order_id IS DISTINCT FROM existing_fill.order_id
                   OR NEW.portfolio_id IS DISTINCT FROM existing_fill.portfolio_id
                   OR NEW.symbol IS DISTINCT FROM existing_fill.symbol
                   OR NEW.side IS DISTINCT FROM existing_fill.side
                   OR NEW.quantity IS DISTINCT FROM existing_fill.quantity
                   OR NEW.price IS DISTINCT FROM existing_fill.price
                   OR NEW.fee IS DISTINCT FROM existing_fill.fee
                   OR NEW.currency IS DISTINCT FROM existing_fill.currency
                   OR NEW.filled_at IS DISTINCT FROM existing_fill.filled_at
                   OR NEW.source_bar_time IS DISTINCT FROM existing_fill.source_bar_time
                   OR NEW.execution_policy_version_id IS DISTINCT FROM
                       existing_fill.execution_policy_version_id
                   OR NEW.reversal_of_id IS DISTINCT FROM existing_fill.reversal_of_id THEN
                    RAISE EXCEPTION 'paper fill idempotency key reused with different facts';
                END IF;
                RETURN NEW;
            END IF;
